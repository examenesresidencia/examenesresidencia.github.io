...
// ========== CÓDIGO NUEVO - AGREGAR DESPUÉS DEL h3 ==========
      ...
      // Se crea siempre para que el admin pueda agregar/editar explicación e imágenes
      {
        ...
        explicacionDiv.style.overflow = "hidden";
  ...
  // ── Construir índice progresivamente ────────────────────────────
  function bBuildIndex(onProgress, onDone) {
    ...
    // Excluir 'simulador': sus preguntas ya están indexadas en sus secciones originales
    const secciones = Object.keys(preguntasPorSeccion || {}).filter(s => s !== 'simulador');
...
<div class="buscador-card${visClass}" data-card-id="${cardId}"
  ...
  function bOnInput() {
    ...
    debounceTimer = setTimeout(() => {
      if (!indexBuilt) {
        bSetStatus('⏳ Construyendo índice…');
        bBuildIndex(
          (done, total) => bSetStatus(`⏳ Cargando base de preguntas (${done}/${total})…`),
          () => {
  ...
  window.mostrarBuscador = function () {
    ...
    if (lastQuery.length >= 2) {
      if (!indexBuilt) {
        bSetStatus(`⏳ Cargando base de preguntas…`);
        bBuildIndex(
          (done, total) => bSetStatus(`⏳ Cargando base de preguntas (${done}/${total})…`),
          () => {
        ...
        bBuildIndex(
          (done, total) => {
            ...
            if (panel2 && panel2.style.display !== 'none') {
              bSetStatus(`⏳ Cargando base de preguntas (${done}/${total})…`);
            }
          },
          () => {
            const panel2 = document.getElementById('buscador-panel');
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const origVolverAlMenu = window.volverAlMenu;
    ...
    window.volverAlMenu = function () {
      // Si hay progreso parcial, el modal de showMenu/origVolverAlMenu se encarga
      // de pedir confirmación; solo limpiar UI del buscador si se confirma la salida
      ...
      if (hayProgresoParcial) {
        // El modal lo maneja origVolverAlMenu → showMenu; conectamos la limpieza
        // del buscador como parte de la acción de confirmación
      ...
      limpiarUIBuscador();
      if (origVolverAlMenu) origVolverAlMenu();
    };
  });
        ...
        try {
          ...
          if (snap.exists()) {
            ...
            if (status === 'approved') {
              ...
              // Siempre volver al menú principal al iniciar sesión
              showMenu();
  ...
  // ── Loading shield: ocultar todo el contenido hasta que Firebase
  // confirme la sesión (evita el flash de UI sin estilos al recargar)
  (function instalarLoadingShield() {
    if (document.getElementById('fb-loading-shield')) return;
    const shield = document.createElement('div');
    shield.id = 'fb-loading-shield';
    ...
    shield.innerHTML = `
      <div style="width:44px;height:44px;border:3px solid rgba(56,189,248,0.2);
        border-top-color:#38bdf8;border-radius:50%;
        animation:shieldSpin 0.7s linear infinite;"></div>
      <style>@keyframes shieldSpin{to{transform:rotate(360deg);}}</style>`;
    document.body.appendChild(shield);
    window._fbLoadingShield = shield;
  })();

  // Quitar el shield: se llama después de que Firebase resuelve la sesión
  ...
  // Safety net: si Firebase no resuelve en 5s (sin conexión, etc.), quitar el shield igual
  setTimeout(() => quitarLoadingShield(), 5000);
  ...
  function abrirModalEdicionAdmin(seccionId, qIndex) {
    ...
    overlay.innerHTML = `
      <div class="fb-card" style="max-width:620px;width:100%;max-height:92vh;overflow-y:auto;">
        ...
        <div class="fb-field" style="margin-bottom:4px;">
          <!-- Toolbar de la explicación con botón imagen -->
    ...
    btnImg.addEventListener('click', () => {
      ...
      panel.innerHTML = `
        ...
        <div class="meq-input-row">
          <div class="meq-prefix">imagenes/</div>
          <input class="meq-input" id="meq-nombre" type="text"
            placeholder="ej: pediatria_displasia_cadera.jpg"
            autocomplete="off" spellcheck="false"/>
        </div>
      ...
      // Bloquear propagación de teclas para no cerrar el modal
      inputNombre.addEventListener('keydown', e => {
        e.stopPropagation();
        if (e.key === 'Enter') { e.preventDefault(); btnVerificar.click(); }
        if (e.key === 'Escape') { panelCont.innerHTML = ''; btnImg.style.background=''; btnImg.style.borderColor=''; }
      });
      inputNombre.addEventListener('mousedown', e => e.stopPropagation());
      ...
      // Insertar — pone el HTML de imagen en el cursor del textarea de explicación
      btnInsertar.addEventListener('click', () => {
        ...
        // Insertar en la posición del cursor del textarea
        ...
        ta.value = antes + '\n' + imgHtml + '\n' + despues;
        ta.selectionStart = ta.selectionEnd = start + imgHtml.length + 2;
        ta.focus();
